...
### Sequences from MPRA variations and oligo-analyisis background model

```{bash, eval=FALSE}
##Obtain TF matrices 
grep -i -E 'GATA1|KLF1|DHS|TAL1|ETS|FLI1|AP-1
|NFE2|LDB1' data/matrices/clusters_motif_names.tab | cut -f1 > data/matrixnames.list
...
#variation-scan for all matrices for TFs relevant for erythrocytes
...
This step is time consuming, precomputed files are avialble in the folder *data/Precomputed*

### Read MPRA data
Read in raw barcode count data for MPRA:
```{r, eval=FALSE, include=TRUE}
dir <- getwd() ####SET DIRECTORY but default we use the local directoy where the Rmarkdown was started
MPRA <-
...
```{r eval = FALSE}
### Permuted matrices
vs_filtered <-  vs[vs$posconcat %in% mpra_sig$posconcat, ]
mpra_tmp <- tapply(mpra$CTRL.mut.p, mpra$posconcat, mean)
max_mpra <- data.frame(posconcat = names(mpra_tmp), mpra = log10(mpra_tmp))

vs_filtered_tmp <- tapply(vs_filtered$pval_ratio, vs_filtered$posconcat, mean)
max_vs <- data.frame(posconcat = names(vs_filtered_tmp), vs = vs_filtered_tmp)

vars <- merge(max_mpra, max_vs, by = "posconcat")




pdf("Correlation_pval_w-diff.pdf")
ggscatter(vars, x = "mpra", y = "vs", 
          add = "reg.line", conf.int = TRUE, 
          cor.coef = TRUE, cor.method = "pearson",
          xlab = "MPRA p-value for differential activity (log10)", ylab = "Variation-scan p-value ratio")
dev.off()
